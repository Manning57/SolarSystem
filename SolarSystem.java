...
public class SolarSystem extends JFrame implements GLEventListener {
	...
	public void display(GLAutoDrawable drawable) {
		...
		//mvStack.translate(Math.cos(camera.getXRot()*Math.PI/18)*(-camera.getXPos()), Math.cos(camera.getYRot()*Math.PI/18)*(-camera.getYPos()), -camera.getZPos());
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (keyCode == 65) {// a, strafe left
					...
					//handling rotating left and right
					double d = -0.0005*(Math.sin(camera.getYRot()*Math.PI/18));
					camera.setZPos((float) (camera.getZPos() + d));	
				}
				...
				if (keyCode == 87) {// w, move forward
					...
					//handling rotating left and right
					double d = 0.0005*(Math.sin(camera.getYRot()*Math.PI/18));
					camera.setXPos((float) (camera.getXPos() + d));
					
					//handling rotating up and down
					double f = 0.0005*(Math.sin(camera.getXRot()*Math.PI/18));
					camera.setYPos((float) (camera.getYPos() + f));					
				}
				...
				//spacebar toggles axes visibility
				if (keyCode == 32) {
					if (vis > 0) {
				...
				//pressing m repositions the camera to the starting position
				if (keyCode == 77) {
					camera.setupCamera(0, 0, 12, 0, 0, 0);
				}
			}
		});
	}
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		try {
			TextureData topFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/top.jpg"), false, "jpg");
			TextureData leftFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/left.jpg"), false, "jpg");
			TextureData fntFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/center.jpg"), false, "jpg");
			TextureData rightFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/right.jpg"), false, "jpg");
			TextureData bkFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/back.jpg"), false, "jpg");
			TextureData botFile = TextureIO.newTextureData(glp, new File("prog_data/Prog9_2_data/bottom.jpg"), false, "jpg");
	...
	private int createShaderProgram() {
		...
		String vshaderSource[] = util.readShaderSource("prog_data/Prog6_1b_data/vert.shader");
		String fshaderSource[] = util.readShaderSource("prog_data/Prog6_1b_data/frag.shader");
